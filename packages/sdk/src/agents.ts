...
import { createRepoLens } from './index'
import type { RepoLensPreset, ApprovalConfig } from './index'
...
const PRESET_INSTRUCTIONS: Record<RepoLensPreset, string> = {
...
export function createGithubAgent({
  ...
  const tools = createRepoLens({ token, requireApproval, preset })
