...
export const metadata: Metadata = {
  title: 'Configuration',
  description: 'configure blocklist, prompt, and model for triggers.',
};
...
export default function Config() {
  return (
    <article className="py-12">
      <Header
        section="Reference"
        title="Configuration"
        description="Use .github/triggers.yml to tell triggers how to classify, which model to use, and which labels must never be applied automatically."
      />

      <Section id="file" title="File Location">
        <p className="text-base leading-7 text-white/62">
          Triggers reads an optional repo config from:
        </p>
        <Codeinline>.github/triggers.yml</Codeinline>
        <Note title="Default Behavior">
          if the file is missing, triggers still runs. it reads label names and
      ...
      <Section id="surfaces" title="The Three Config Surfaces">
        <Grid>
          <Card
            ...
            description="top-level labels that triggers must never add on its own."
          />
          <Card
            title="Prompt"
  ...
  setup questions usually map to support.`}</Code>
      ...
      <Section id="model" title="Model">
        <p className="text-base leading-7 text-white/62">
          Triggers defaults to ai sdk 6 with{' '}
          <code className="text-accent">anthropic/claude-sonnet-4.6</code>.
        </p>
        <p className="mt-3 text-base leading-7 text-white/62">
          When you use the default model, triggers prefers bedrock through ai
