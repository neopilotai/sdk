export function Empty({ hasrepos }: { hasrepos: boolean }) {
  return (
    <div className="grid h-full min-h-0 place-items-center p-4 md:p-8">
      <div className="w-full rounded-[2rem] border border-border bg-[radial-gradient(circle_at_top_left,rgba(212,228,92,0.18),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0))] px-6 py-10 text-center md:px-10 md:py-12">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-border bg-bg/70">
          <svg
            className="h-7 w-7 text-fg/70"
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight md:text-4xl">
          {hasrepos
            ? 'Pick a repository to review'
            : 'No repositories connected yet'}
        </h1>
        <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-muted">
          {hasrepos
            ? 'Select a repo from the left rail to open activity, memory, and config in the same workspace.'
            : 'Install Triggers on a repository to start triage, memory, and operator review from this dashboard.'}
        </p>
        {!hasrepos ? (
          <a
            href="https://github.com/apps/triggers"
            className="mt-6 inline-flex items-center rounded-2xl bg-fg px-5 py-3 text-sm font-medium text-bg transition-opacity hover:opacity-85"
          >
            Install Triggers
          </a>
        ) : null}
