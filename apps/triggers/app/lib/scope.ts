...
export function readscope(): Scope {
  const owners = parse(process.env.TRIGGERS_ALLOWED_OWNERS);
  return {
    owners: owners.length > 0 ? owners : ['vercel', 'triggers'],
    repos: parse(process.env.TRIGGERS_ALLOWED_REPOS).map(item =>
      item.toLowerCase(),
    ),
  };
}
