...
export async function createpr(gh: Gh, learn: Learn, config: Config) {
  ...
  const branch = `triggers/learn-${learn.number}`;
  ...
  await gh.octokit.rest.repos.createOrUpdateFileContents({
    ...
    repo: gh.repo,
    path: '.github/triggers.yml',
    message: `fix: update triggers config from #${learn.number}`,
  ...
  const body = [
    `updates \\.github/triggers.yml\\ from #${learn.number}.`,
    '',
