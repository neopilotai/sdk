...
export async function planmessage(
  ...
  const { output } = await generateText({
    ...
    system: `you plan actions for triggers, a github labeling agent.

capabilities:
- explain why labels were chosen
- add labels
- remove labels
- update the repo blocklist in triggers.yml
...
- when a maintainer directly asks to add a specific label, include it in add even if the repo blocklist will stop triggers from applying it later
...
- put labels in block when the maintainer says triggers should never add them, should leave them to humans, or should add them manually
- when the maintainer says triggers should leave a currently applied label to humans, also remove that label
- set learn to true when the maintainer asks triggers to learn, remember, update its config, update its rules, or make a pr
...
export async function handlecomment(
  ...
  const matchtext = body.match(/^@triggers\s+(.+)/is);
  if (!matchtext) return;
  ...
  if (parts.length === 0) {
    parts.push(
      'tell me which labels to add, remove, explain, or keep out of triggers.',
    );
  }
